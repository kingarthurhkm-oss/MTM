...
for row in range(240):
    for col in range(120):
        lon = 118 + col / 119 * 29
        y = top - (row + (col % 2) * 0.5) / 239.5 * (top - bottom)
...
for name, lon, lat in islands:
    col = round((lon - 118) / 29 * 119)
    row = round((top - merc(lat)) / (top - bottom) * 239.5 - (col % 2) * .5)
    cells[row * 120 + col] = 1
rle = []
for cell in cells:
    if rle and rle[-2] == cell:
        ...
        rle.extend([cell, 1])
data = {'columns': 120, 'rows': 240, 'bounds': [118, 24, 147, 46],
