...
(async()=>{
 ...
 try{
  ...
  await page.locator('[data-modal="reset-army"]').click();
  assert.equal(await page.locator('#unit-count').innerText(),'64');
  await page.locator('[data-tab="forces"]').click();
  ...
  assert.match(await page.locator('#panel-content h3').innerText(),/15보병사단/);
  await page.locator('#end-turn').click();
  await page.waitForFunction(()=>document.getElementById('turn-value').textContent==='02');
  await page.reload();
  assert.equal(await page.locator('#unit-count').innerText(),'64');
  ...
  await standalone.locator('[data-modal="reset-army"]').click();
  assert.equal(await standalone.locator('#unit-count').innerText(),'64');
  await standalone.locator('[data-tab="forces"]').click();
  await standalone.locator('[data-unit="blue-rok-bde-art-1"]').click();
  assert.match(await standalone.locator('#panel-content').innerText(),/제1포병여단/);
  await standalone.locator('#end-turn').click();
  await standalone.waitForFunction(()=>document.getElementById('turn-value').textContent==='02');
  assert.deepEqual(errors,[]);
