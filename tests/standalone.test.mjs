...
test('distributed HTML runs the 72x126 default Army game and retires stale downloads',()=>{
 ...
 assert.ok(html.includes(`<title>PENINSULA 2026 ${release} · 72×126</title>`));
 ...
 assert.equal(g.board.cols,72);assert.equal(g.board.rows,126);
 assert.equal(g.board.tiles.length,9072);
